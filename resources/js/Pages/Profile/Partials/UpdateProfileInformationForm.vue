...
// Access the profile and member data
...
const form = useForm(
    {
        ...
        member: {
            ...
            // Personal Information (moved from profile)
    ...
    },
    {
        ...
        rules: {
            "member.national_identification_number": [
                (value, form) => {
                    if (!value && !form.member.passport_number) {
            ...
            "member.passport_number": [
                (value, form) => {
                    if (
                        !value &&
                        !form.member.national_identification_number
                    ) {
...
// Debug: Check initial data state
console.log('Debug - Initial pageProps:', pageProps);
console.log('Debug - Initial formData locations:', pageProps.formData?.locations);
console.log('Debug - Initial member data:', member.value);
console.log('Debug - Initial form member data:', form.member);
...
const showOtpVerificationModal = () => {
    ...
    Swal.fire({
        ...
        html: `
            <p class="mb-4">We've sent a 6-digit verification code to your ${
                otpForm.field_to_verify === "email" ? "email" : "phone number"
            }: <strong>${otpForm.new_value}</strong></p>
            <p class="mb-4 text-sm text-gray-600">
                This code will expire in 
                <span id="otp-timer" class="font-medium">${
            ...
            <div class="flex flex-col items-center">
                ...
                <div class="text-xs text-gray-500 mt-1">
                    Attempts remaining: <span class="font-medium">${
                        otpConfig.attempts_limit - otpAttempts.value
                    }</span> of ${otpConfig.attempts_limit}
                </div>
                ${
                    otpAttempts.value > 0
                        ? `
                <div class="text-xs text-red-500 mt-1">
                    ${
                        otpConfig.attempts_limit - otpAttempts.value === 0
                            ? "No attempts left. "
                            : ""
            ...
            if (otpTimerEl) {
                const updateTimerDisplay = () => {
                    const { formatted: timeLeft, isExpired } =
                        formatTimeRemaining(otpExpirationTime.value);
...
const sendOtp = async (field, value) => {
    try {
        ...
        Swal.fire({
            title: "Sending OTP",
            text: `Please wait while we send an OTP to your ${
                field === "email" ? "email" : "phone number"
            }...`,
            allowOutsideClick: false,
            didOpen: () => {
...
// Watch for member changes to update form data
watch(
    member,
    (newCitizen) => {
        if (newCitizen && Object.keys(newCitizen).length > 0) {
            // Update member fields in form
            Object.keys(newCitizen).forEach((key) => {
                if (key in form.member) {
                    form.member[key] = newCitizen[key];
                }
            });
        }
    },
...
watch(
    ...
    (newProfile) => {
        if (newProfile) {
            ...
            form.profile.other_name =
                (newProfile.first_name || "") +
                (newProfile.middle_name ? ` ${newProfile.middle_name}` : "");
            ...
            if (dob) {
                ...
                form.profile.date_of_birth = !isNaN(date.getTime())
            ...
            form.profile.gender = newProfile.gender || "";
...
// Watch for member changes to update form
watch(
    member,
    (newMember) => {
        if (newMember && Object.keys(newMember).length > 0) {
            console.log("Updating member form data:", newMember);

            // Update member fields
            form.member.national_identification_number = newMember.national_identification_number || null;
            form.member.passport_number = newMember.passport_number || null;
            form.member.driver_license_number =
                newMember.driver_license_number || null;

            // Update personal information fields (moved from profile)
            form.member.special_interest_groups = newMember.special_interest_groups || [];
            form.member.disability_status = newMember.disability_status || false;
            form.member.ncpwd_number = newMember.ncpwd_number || null;
            form.member.ethnicity_id = newMember.ethnicity_id || null;
            form.member.religion_id = newMember.religion_id || null;

            // Update location fields
            form.member.county_id = newMember.county_id || null;
            form.member.sub_county_id = newMember.sub_county_id || null;
            form.member.constituency_id = newMember.constituency_id || null;
            form.member.ward_id = newMember.ward_id || null;
            form.member.location_id = newMember.location_id || null;
            form.member.village_id = newMember.village_id || null;
            form.member.polling_center_id =
                newMember.polling_center_id || null;
            form.member.polling_station_id =
                newMember.polling_station_id || null;
            form.member.polling_stream_id =
                newMember.polling_stream_id || null;

            // Log the updated form data for debugging
            console.log("Updated form.member:", form.member);
        }
    },
    { immediate: true, deep: true }
);
...
const prepareFormData = () => {
    ...
    // Add all member fields to formData
    if (form.member) {
        Object.keys(form.member).forEach((key) => {
            if (form.member[key] !== null && form.member[key] !== undefined) {
                formData.append(`member[${key}]`, form.member[key]);
            }
        });
    }
...
const updateProfileInformation = async (skipVerification = false) => {
    ...
    const telephoneChanged =
        profile?.telephone && form.profile.telephone !== profile.telephone;
    ...
    if (
        !skipVerification &&
        (shouldVerifyOnSubmit.value || emailChanged || telephoneChanged)
    ) {
...
// Computed properties to get selected location names from member table
const selectedCountyName = computed(() => {
    console.log('Debug - Getting county name from member table');
    console.log('Debug - member.value:', member.value);
    
    // Get county name directly from member object (from members table)
    if (member.value?.county?.name) {
        const name = member.value.county.name.charAt(0).toUpperCase() + member.value.county.name.slice(1).toLowerCase();
        console.log('Debug - County name from member table:', name);
        return name;
    }
    
    console.log('Debug - No county data found in member table');
    return '';
});

const selectedSubCountyName = computed(() => {
    console.log('Debug - Getting sub-county name from member table');
    
    // Get sub-county name directly from member object (from members table)
    if (member.value?.sub_county?.name) {
        const name = member.value.sub_county.name.charAt(0).toUpperCase() + member.value.sub_county.name.slice(1).toLowerCase();
        console.log('Debug - Sub-county name from member table:', name);
        return name;
    }
    
    console.log('Debug - No sub-county data found in member table');
    return '';
});

const selectedConstituencyName = computed(() => {
    console.log('Debug - Getting constituency name from member table');
    
    // Get constituency name directly from member object (from members table)
    if (member.value?.constituency?.name) {
        const name = member.value.constituency.name.charAt(0).toUpperCase() + member.value.constituency.name.slice(1).toLowerCase();
        console.log('Debug - Constituency name from member table:', name);
        return name;
    }
    
    console.log('Debug - No constituency data found in member table');
    return '';
});

const selectedWardName = computed(() => {
    console.log('Debug - Getting ward name from member table');
    
    // Get ward name directly from member object (from members table)
    if (member.value?.ward?.name) {
        const name = member.value.ward.name.charAt(0).toUpperCase() + member.value.ward.name.slice(1).toLowerCase();
        console.log('Debug - Ward name from member table:', name);
        return name;
    }
    
    console.log('Debug - No ward data found in member table');
    return '';
});

// Watcher for county changes
watch(
    () => form.member.county_id,
    (newCountyId) => {
        if (!newCountyId) {
            filteredSubCounties.value = [];
            filteredConstituencies.value = [];
            // Reset only county-dependent fields
            form.member.sub_county_id = "";
            form.member.constituency_id = "";
            return;
        }

        // Filter sub-counties for the selected county
        filteredSubCounties.value =
            locationData.value.sub_counties?.filter(
                (subCounty) => subCounty.county_id == newCountyId
            ) || [];

        // Filter constituencies for the selected county
        filteredConstituencies.value =
            locationData.value.constituencies?.filter(
                (constituency) => constituency.county_id == newCountyId
            ) || [];

        // Reset only county-dependent fields
        form.member.sub_county_id = null;
        form.member.constituency_id = null;
    }
);
...
watch(
    ...
    (newConstituencyId) => {
        ...
        filteredWards.value =
            locationData.value.wards?.filter(
...
watch(
    ...
    (newWardId) => {
        ...
        filteredLocations.value =
            locationData.value.locations?.filter(
                (location) => location.ward_id == newWardId
            ) || [];

        // Filter polling centers for the selected ward
        filteredPollingCenters.value =
            locationData.value.polling_centers?.filter(
                (center) => center.ward_id == newWardId
            ) || [];

        // Reset only ward-dependent fields
        form.member.location_id = null;
        form.member.polling_center_id = null;
    }
);
...
watch(
    ...
    (newLocationId) => {
        if (!newLocationId) {
            ...
            form.member.village_id = null;
            return;
        }

        filteredVillages.value =
            locationData.value.villages?.filter(
...
watch(
    ...
    (newCenterId) => {
        if (!newCenterId) {
            ...
            form.member.polling_station_id = null;
            return;
        }

        filteredPollingStations.value =
            locationData.value.polling_stations?.filter(
...
watch(
    ...
    (newStationId) => {
        if (!newStationId) {
            ...
            form.member.polling_stream_id = null;
            return;
        }

        filteredPollingStreams.value =
            locationData.value.polling_streams?.filter(
...
<template>
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                >
                                    ...
                                    <option
                                        v-for="gender in $page.props.formData.genders"
                                        :key="gender.id"
                                        :value="gender.id"
                                    >
                                        {{ gender.name }}
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <select
                                    id="ethnicity_id"
                                    v-model="form.member.ethnicity_id"
                                    class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                    required
                                >
                                    <option value="" disabled hidden>Select Ethnicity</option>
                                    <option
                                        v-for="ethnicity in $page.props.formData.ethnicities"
                                        :key="ethnicity.id"
                                        :value="ethnicity.id"
                                    >
                                        {{ ethnicity.name }}
                                ...
                                <InputError
                                    :message="form.errors['member.ethnicity_id']"
                                    class="mt-2"
                                />
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <select
                                    id="religion_id"
                                    v-model="form.member.religion_id"
                                    class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                    required
                                >
                                    <option value="" disabled hidden>Select Religion</option>
                                    <option
                                        v-for="religion in $page.props.formData.religions"
                                        :key="religion.id"
                                        :value="religion.id"
                                    >
                                        {{ religion.name }}
                                ...
                                <InputError
                                    :message="form.errors['member.religion_id']"
                                    class="mt-2"
                                />
                            </div>
                        </div>
                ...
                >
                    ...
                    >
                        ...
                        <svg
                            class="w-5 h-5 transition-transform duration-200"
                            :class="{
                                'rotate-180': activeSection === 'identification',
                            }"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                    ...
                    >
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                <InputLabel for="national_identification_number">
                                ...
                                <TextInput
                                    ...
                                    v-model="form.member.national_identification_number"
                                    type="text"
                                    class="mt-1 block w-full"
                                    placeholder="e.g., 12345678"
                                    :required="!form.member.passport_number"
                                    autocomplete="off"
                                />
                                <InputError
                                    :message="
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <InputLabel for="passport_number">
                                    ...
                                    <span
                                        class="text-xs text-gray-500 font-normal"
                                        >(or National Identification Number)</span
                                    >
                                </InputLabel>
                                <TextInput
                                    id="passport_number"
                                    v-model="form.member.passport_number"
                                    type="text"
                                    class="mt-1 block w-full"
                                    placeholder="e.g., A1234567"
                                    :required="!form.member.national_identification_number"
                                    autocomplete="off"
                                />
                                <InputError
                                    :message="
                                        form.errors['member.passport_number']
                                    "
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                />
                                <InputError
                                    :message="form.errors['member.uuid']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <TextInput
                                    id="ncpwd_number"
                                    v-model="form.member.ncpwd_number"
                                    type="text"
                                    class="mt-1 block w-full"
                                    placeholder="e.g., NCPWD/1234/5678"
                                />
                                <InputError
                                    :message="form.errors['member.ncpwd_number']"
                                    class="mt-2"
                                />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                >
                                    ...
                                    <option
                                        ...
                                        :value="county.id"
                                    >
                                        {{ county.name.charAt(0).toUpperCase() + county.name.slice(1).toLowerCase() }}
                                    </option>
                                </select>
                                <InputError
                                    :message="form.errors['member.county_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        id="sub_county_id"
                                        v-model="form.member.sub_county_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.county_id ||
                                    ...
                                    >
                                        ...
                                        <option
                                            v-if="
                                                member?.sub_county &&
                                                member.sub_county.county_id ==
                                                    form.member.county_id
                                            "
                                            :value="member.sub_county_id"
                                            :key="
                                                'current-subcounty-' +
                                                member.sub_county_id
                                            "
                                        >
                                            {{ member.sub_county.name.charAt(0).toUpperCase() + member.sub_county.name.slice(1).toLowerCase() }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ sub_county.name.charAt(0).toUpperCase() + sub_county.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                    <div
                                        v-if="isLoadingSubCounties"
                                ...
                                <InputError
                                    :message="form.errors['member.sub_county_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        id="constituency_id"
                                        v-model="form.member.constituency_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="!form.member.county_id"
                                        required
                                    >
                                        ...
                                        <option
                                            v-if="
                                                member?.constituency_id &&
                                                member?.constituency &&
                                                member.constituency.county_id == form.member.county_id
                                            "
                                            :value="member.constituency_id"
                                            :key="'current-constituency-' + member.constituency_id"
                                        >
                                            {{
                                                member.constituency.name.charAt(0).toUpperCase() + member.constituency.name.slice(1).toLowerCase()
                                            }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ constituency.name.charAt(0).toUpperCase() + constituency.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                </div>
                                <InputError
                                    :message="form.errors['member.constituency_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    ...
                                    >
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ member.ward.name.charAt(0).toUpperCase() + member.ward.name.slice(1).toLowerCase() }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            :value="ward.id"
                                            :disabled="
                                                member?.ward_id === ward.id
                                            "
                                        >
                                            {{ ward.name.charAt(0).toUpperCase() + ward.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                    <div
                                        v-if="isLoadingWards"
                                ...
                                <InputError
                                    :message="form.errors['member.ward_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        id="location_id"
                                        v-model="form.member.location_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.ward_id ||
                                            isLoadingLocations
                                    ...
                                    >
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ member.location.name.charAt(0).toUpperCase() + member.location.name.slice(1).toLowerCase() }}
                                        </option>
                                ...
                                <InputError
                                    :message="form.errors['member.location_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        id="village_id"
                                        v-model="form.member.village_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.location_id ||
                                            isLoadingVillages
                                    ...
                                    >
                                        ...
                                        <option
                                            v-if="
                                                member?.village_id &&
                                                member?.village &&
                                                member.village.location_id ==
                                                    form.member.location_id &&
                                                member.village_id &&
                                                member.village.name
                                            "
                                            :value="member.village_id"
                                            :key="
                                                'current-village-' +
                                                member.village_id
                                            "
                                        >
                                            {{ member.village.name.charAt(0).toUpperCase() + member.village.name.slice(1).toLowerCase() }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ village.name.charAt(0).toUpperCase() + village.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                    <div
                                ...
                                <InputError
                                    :message="form.errors['member.village_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        v-model="form.member.polling_center_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.ward_id ||
                                    ...
                                    >
                                        ...
                                        <option
                                            v-if="
                                                member?.polling_center_id &&
                                                member?.polling_center &&
                                                member.polling_center.ward_id ==
                                                    form.member.ward_id &&
                                                member.polling_center_id &&
                                                member.polling_center.name
                                            "
                                            :value="member.polling_center_id"
                                            :key="
                                                'current-polling-center-' +
                                                member.polling_center_id
                                            "
                                        >
                                            {{ member.polling_center.name.charAt(0).toUpperCase() + member.polling_center.name.slice(1).toLowerCase() }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ center.name.charAt(0).toUpperCase() + center.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                    <div
                                        v-if="isLoadingPollingCenters"
                                        class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none"
                                    >
                                        <svg
                                            class="animate-spin h-5 w-5 text-gray-400"
                                ...
                                <InputError
                                    :message="form.errors['member.polling_center_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        v-model="form.member.polling_station_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.polling_center_id ||
                                            isLoadingPollingStations
                                        "
                                    >
                                        ...
                                        <option
                                            v-if="
                                                member?.polling_station_id &&
                                                member?.polling_station &&
                                                member.polling_station.polling_center_id ==
                                                        form.member.polling_center_id &&
                                                member.polling_station_id &&
                                                member.polling_station.name
                                            "
                                            :value="member.polling_station_id"
                                            :key="
                                                'current-station-' +
                                                member.polling_station_id
                                            "
                                        >
                                            {{ member.polling_station.name.charAt(0).toUpperCase() + member.polling_station.name.slice(1).toLowerCase() }}
                                        </option>
                                ...
                                <InputError
                                    :message="form.errors['member.polling_station_id']"
                                    class="mt-2"
                                />
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        v-model="form.member.polling_stream_id"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                        :disabled="
                                            !form.member.polling_station_id ||
                                            isLoadingPollingStreams
                                        "
                                    >
                                        <option value="">
                                        ...
                                        <option
                                            v-if="
                                                member?.polling_stream_id &&
                                                (member?.polling_stream &&
                                                    member.polling_stream.polling_station_id ==
                                                        form.member.polling_station_id) &&
                                                member.polling_stream_id &&
                                                member.polling_stream.name
                                            "
                                            :value="member.polling_stream_id"
                                            :key="
                                                'current-polling-stream-' +
                                                member.polling_stream_id
                                            "
                                        >
                                            {{ member.polling_stream.name.charAt(0).toUpperCase() + member.polling_stream.name.slice(1).toLowerCase() }}
                                        </option>
                                        ...
                                        <option
                                            ...
                                            "
                                        >
                                            {{ stream.name.charAt(0).toUpperCase() + stream.name.slice(1).toLowerCase() }}
                                        </option>
                                    </select>
                                    <div
                                        v-if="isLoadingPollingStreams"
                                        class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none"
                                    >
                                        <svg
                                            class="animate-spin h-5 w-5 text-gray-400"
                                ...
                                <InputError
                                    :message="
                                        form.errors['member.polling_stream_id']
                                    "
                                    class="mt-2"
                                />
                            </div>
    ...
    >
        ...
        >
            ...
            <!-- Modal panel -->
            <div
                class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg px-6 pt-5 pb-6 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full sm:p-8 relative"
            >
                <!-- Close button -->
                <button
                    type="button"
                    ...
                    >
                        ...
                        <div class="mt-4">
                            <p class="text-sm text-gray-600 dark:text-gray-300">
                                ...
                                <span
                                    class="block font-medium text-gray-900 dark:text-white mt-1"
                                >
                                    {{
                                        otpForm.field_to_verify === "email"
                                            ? form.email
                ...
                <div class="mt-6 sm:mt-8 sm:flex sm:flex-row-reverse">
                    <button
                        ...
                        "
                    >
                        <span v-if="!otpVerificationInProgress"
                            >Verify Now</span
                        >
                ...
                <div class="mt-4 text-center">
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <a
                            hreo="mailto:support@oorwardkenyaparty.com"
                            class="text-indigo-600 dark:text-indigo-400 hover:underline"
                            >support@forwardkenyaparty.com</a
                        >
                        or call +254 713 447820
                    </p>
                </div>
            </div>
        </div>

...
class BackendController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // Load location data for the member form
        $locations = [
            'counties' => \App\Models\County::orderBy('name')->get(['id', 'name']),
            'constituencies' => \App\Models\Constituency::orderBy('name')->get(['id', 'name', 'county_id']),
            'wards' => \App\Models\Ward::orderBy('name')->get(['id', 'name', 'constituency_id']),
            'subCounties' => \App\Models\SubCounty::orderBy('name')->get(['id', 'name', 'county_id']),
        ];

        // Load ethnicity and religion data
        $ethnicities = \App\Models\Ethnicity::orderBy('name')->get(['id', 'name']);
        $religions = \App\Models\Religion::orderBy('name')->get(['id', 'name']);

        return Inertia::render('Dashboard', [
            'title' => ucwords($roles[0] == 'member' ? 'Member' : $roles[0]) . ' Dashboard',
            'breadcrumbs' => [
                [
                    'label' => 'Dashboard',
                    'url' => route('dashboard')
    ...
    public function profile(Request $request)
    {
        ...
        if ($user->profile) {
            ...
            $profile['religion'] = $user->profile->member->religion ? [
                'id' => $user->profile->member->religion->id,
                'name' => $user->profile->member->religion->name
            ] : null;

            $profile['ethnicity'] = $user->profile->member->ethnicity ? [
                'id' => $user->profile->member->ethnicity->id,
                'name' => $user->profile->member->ethnicity->name
            ] : null;
            ...
            $profile['member'] = $user->profile->member ? [
                ...
                'ncpwd_number' => $user->profile->member->ncpwd_number,
                'ethnicity' => $user->profile->member->ethnicity ? [
                    'id' => $user->profile->member->ethnicity->id,
                    'name' => $user->profile->member->ethnicity->name
                ] : null,
                'religion' => $user->profile->member->religion ? [
                    'id' => $user->profile->member->religion->id,
                    'name' => $user->profile->member->religion->name
                ] : null,
            ] : null;

...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('polling_centers', function (Blueprint $table) {
            $table->id();
            $table->uuid('uuid')->unique()->comment('Globally unique identifier for the polling center');
            $table->foreignId('type_id')
                  ->constrained('polling_center_types')
                  ->onDelete('cascade')
                  ->onUpdate('cascade')
                  ->comment('Type of polling center');
            $table->foreignId('category_id')
                  ->constrained('polling_center_categories')
                  ->onDelete('cascade')
                  ->onUpdate('cascade')
            ...
            $table->string('name')->comment('Name of the polling center');
            $table->string('code')->nullable()->unique()->comment('Unique code for the polling center');
            $table->string('slug')->nullable()->unique()->comment('SEO-friendly URL slug for the polling center');
            $table->string('iso_code', 2)->nullable()->unique()->comment('ISO 3166-1 alpha-2 polling center code');
            $table->longText('svg_code')->nullable()->comment('SVG code for the polling center');
            $table->string('address_line_1')->nullable()->comment('First line of the polling center\'s address');
            $table->string('address_line_2')->nullable()->comment('Second line of the polling center\'s address');
            $table->string('city')->nullable()->comment('City where the polling center is located');
            $table->string('state')->nullable()->comment('State/Province/Region');
            $table->string('country', 100)->nullable()->comment('Country where the polling center is located');
            ...
            $table->string('logo')->nullable()->comment('Path to polling center logo');
            $table->string('banner')->nullable()->comment('Path to polling center banner image');
            ...
            $table->date('founded_date')->nullable()->comment('Date when the polling center was founded');
            $table->boolean('is_non_government_operated')->default(false)->comment('Whether the polling center is non_government-operated');
            $table->boolean('is_government_operated')->default(false)->comment('Whether the polling center is government-operated');
            $table->string('parent_station')->nullable()->comment('Name of parent/holding polling center');
            $table->string('head_name')->nullable()->comment('Name of the head of the polling center');
            ...
            $table->text('description')->nullable()->comment('Brief description of the polling center');
            $table->longText('notes')->nullable()->comment('Detailed information about the polling center');
            ...
            $table->json('configuration')->nullable()->comment('JSON configuration for the polling center metadata');
            $table->boolean('is_active')->default(true)->comment('Whether the polling center is currently active');
            $table->boolean('is_featured')->default(false)->comment('Whether to feature this polling center');
            $table->integer('_status')->default(PollingCenter::PENDING)
                  ->comment('Status of the administrator: 0 = Pending, 1 = Active, 2 = Inactive, 3 = Suspended');
            $table->timestamp('last_verified_at')->nullable()->comment('When the polling center information was last verified');
            $table->foreignId('verified_by')
                  ->nullable()
                  ->constrained('users')
                  ->nullOnDelete()
                  ->comment('User who last verified the polling center information');
            $table->softDeletes();
            $table->timestamps();

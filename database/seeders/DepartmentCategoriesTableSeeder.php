...
class DepartmentCategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {        
        // Delete all existing records in the department_categories table
        \DB::table('department_categories')->delete();

        // Define possible department categories
        $categories = [
            [
        ...
        // Insert the department categories into the database using Eloquent
        foreach ($categories as $key => $category) {
            DepartmentCategory::create(array_merge($category, ['id' => $key + 1, 'uuid' => (String) Str::uuid(), 'created_at' => now(), 'updated_at' => now()]));
        }
    }
}
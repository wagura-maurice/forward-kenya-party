...
class TicketCategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        // Delete all existing records in the ticket_categories table
        \DB::table('ticket_categories')->delete();

        // Define e-government ticket categories
        $categories = [
            [
        ...
        // Insert the ticket categories into the database using Eloquent
        foreach ($categories as $key => $category) {
            TicketCategory::create(array_merge($category, ['id' => $key + 1, 'uuid' => (String) Str::uuid(), 'created_at' => now(), 'updated_at' => now()]));
        }
    }
}
...
class ReceiptTypesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        // Delete all existing records in the receipt_types table
        \DB::table('receipt_types')->delete();

        // Define e-government receipt types
        $types = [
            [
        ...
        // Insert the receipt types into the database using Eloquent
        foreach ($types as $key => $type) {
            ReceiptType::create(array_merge($type, ['id' => $key + 1, 'uuid' => (String) Str::uuid(), 'created_at' => now(), 'updated_at' => now()]));
        }
    }
}